...
<vxml version="1.0">
	...
	<form id="Begin_callout">
		...
		<!--外呼-->
		<object name="callout2" classid="method://huawei/Call/CallOut">
			<param name="CLI" expr="'112'"/>
			<param name="CLD" expr="'7106'"/>
			<param name="SSP" expr="callee_SSP2"/>

			<filled>				
				<if cond="callout=='HOOK_OFF'">
					<log expr="'hook off and go on ' + callee_SSP2" />
					<clear namelist="joinconf3"/>
				<else/>
					<log expr="'not hook off and return'"/>
				</if>
			</filled>
		...
		<!--被叫用户加入会场-->
		<object name="joinconf3" classid=" method://huawei/Conf/ConfAddUser" expr="true">
			<!-- 会场号 -->
			<param name="ConfNO" expr="curConfNo"/>
			<!-- 连接方SSP -->
			<param name="SrcSSP" expr="callee_SSP2"/>
			<!-- 连接方类型 0表示主叫方,目前只支持设置0 -->
			<param name="ConnPartyType" value="0"/>
			<!-- 返回用户在会场中的通道号 -->
			<param name="ChannelNO" expr="channel3"/>
		</object>
			...
			<filled>
				<if cond="ProcessRBT=='USER_HOOK'">
					<if cond="outputinfo==curSSP">
						<goto next="#callerRelease"/>
					<elseif cond="outputinfo==callee_SSP"/>
						<assign name="curDelChannel" expr="channel2"/>
						<clear namelist="delUser releaseCallSSP"/>
					<elseif cond="outputinfo==callee_SSP2"/>
						<assign name="curDelChannel" expr="channel3"/>
						<clear namelist="delUser releaseCallSSP"/>						
					</if>
				<else/>
	...
	<form id="callerRelease">
		<block>
			...
			<log>outssp2:<value expr="callee_SSP2"/></log>
		</block>
		...
		<object name="releaseCallee2" classid="method://huawei/Call/Disconnect" >
			<!-- 正常拆线 -->
			<param name="Cause" value="0"/>
			<param name="SSP" expr="callee_SSP2"/>
		</object>
