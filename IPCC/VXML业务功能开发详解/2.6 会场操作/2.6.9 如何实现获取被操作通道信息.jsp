...
<vxml version="1.0">
	<form >
            	...
            	<if cond="ProcessCollect=='CP_USERDIALING'">
            		<log>通道收号值为:<value expr="outputinfo"/></log>
            		 <if cond ="outputinfo=='1'">
            		 	<clear namelist="getOperatedChannelInfo stopcollectdigit stopPlay"/>
            		 <else/>
            		 	<clear namelist="CollectCallee ProcessCollect"/>
            		 </if>
            	</if>
       ...
       <var name="curOprtChannelNo"/>
       <var name="curOprtConfNo"/>
       <!-- 获取收号的通道的信息 -->
       <object name=getOperatedChannelInfo classid="method://huawei/Conf/GetOperatedChannelInfo">
			<param name="ChannelNO" expr="curOprtChannelNo"/>
			<param name="ConfNO" expr="curOprtConfNo"/>
			<filled>
				<if cond="getOperatedChannelInfo=='SUCCESS'">
					<log>获取被操作通道信息成功</log>
					<log>会场号:<value expr="curOprtConfNo"/></log>
					<log>通道号号:<value expr="curOprtChannelNo"/></log>
				</if>
			</filled>
		</object>
